...
/**
 * @title UpdateRugNFTFacetDisableValidation
 * @notice Disable all ERC721-C validation to isolate minting issue
 * @dev Completely disables _beforeTokenTransfer validation
 */
contract UpdateRugNFTFacetDisableValidation is Script {
    address constant DIAMOND = 0x2aB6ad4761307CFaF229c75F6B4A909B73175146;
    address constant OLD_RUG_NFT_FACET = address(0); // We'll get this from the diamond

    function run() public {
        uint256 deployerPrivateKey = vm.envUint("PRIVATE_KEY");
        vm.startBroadcast(deployerPrivateKey);

        // Deploy new RugNFTFacet with validation completely disabled
        ...
        console.log("ALL validation temporarily disabled to test minting");
