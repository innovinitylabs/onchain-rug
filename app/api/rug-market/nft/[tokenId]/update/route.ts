...
export async function POST(
  ...
  try {
    ...
    const updateLockKey = `rugmarket:update:lock:${chainId}:${contractAddress}:${tokenId}`
    let lockAcquired = false
    
    try {
      // Try to acquire lock (expires in 30 seconds)
      const lockExists = await redis.get(updateLockKey)
      if (lockExists) {
        return NextResponse.json(
          { 
            error: 'Update already in progress',
            message: 'An update for this NFT is already being processed'
          },
          { status: 409 }
        )
      }
      ...
      // Set lock
      await redis.setex(updateLockKey, 30, '1')
      lockAcquired = true
    } catch (lockError) {
      console.error(`[Update API] Lock check failed:`, lockError)
      // Continue anyway - don't block update if locking fails
    }
    ...
    try {
      ...
      // Release lock
      if (lockAcquired) {
        await redis.del(updateLockKey).catch(() => {})
      }
