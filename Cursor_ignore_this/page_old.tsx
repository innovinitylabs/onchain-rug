...
export default function GeneratorPage() {
  ...
  const [palette, setPalette] = useState<string | undefined>(null)
  ...
  const init = async () => {
    ...
    try {
      ...
      if (typeof (window as any).draw === 'function') {
        ...
        try {
          const doormatFunctions = (window as any) as DoormatFunctions;
          if (typeof doormatFunctions.generateDoormat === 'function') {
            doormatFunctions.generateDoormat(currentSeed.toString());
          }
        } catch (error) {
          console.error('Error generating doormat:', error);
        }
  ...
  const updatePaletteDisplay = () => {
    try {
      const doormatFunctions = (window as any) as DoormatFunctions;
      if (typeof doormatFunctions.getCurrentPalette === 'function') {
        const currentPalette = doormatFunctions.getCurrentPalette();
        if (currentPalette) {
      ...
      console.error('Error getting current palette:', error);
    }
  }
  ...
  const updateTraitsDisplay = () => {
    try {
      const doormatFunctions = (window as any) as DoormatFunctions;
      if (typeof doormatFunctions.calculateTraits === 'function') {
        const currentTraits = doormatFunctions.calculateTraits();
        if (currentTraits) {
  ...
  const generateNew = () => {
    ...
    if (typeof (window as any).draw === 'function') {
      ...
      try {
        const doormatFunctions = (window as any) as DoormatFunctions;
        if (typeof doormatFunctions.generateDoormat === 'function') {
          doormatFunctions.generateDoormat(seed.toString());
        }
      } catch (error) {
        console.error('Error generating doormat:', error);
      }
  ...
  const addTextToDoormat = () => {
    ...
    if (validTexts.length > 0) {
      try {
        const doormatFunctions = (window as any) as DoormatFunctions;
        if (typeof doormatFunctions.addTextToDoormatInSketch === 'function') {
          doormatFunctions.addTextToDoormatInSketch(validTexts);
          console.log('📝 Text added to doormat:', validTexts);
        }
      } catch (error) {
        console.error('Error adding text to doormat:', error);
      }
    }
  }
  ...
  const clearText = () => {
    ...
    try {
      const doormatFunctions = (window as any) as DoormatFunctions;
      if (typeof doormatFunctions.clearTextFromDoormat === 'function') {
        doormatFunctions.clearTextFromDoormat();
      }
    } catch (error) {
      console.error('Error clearing text from doormat:', error);
    }
  }
