...
function drawSelvedgeEdges(){let ws=wt+1,iff=true,il=false;for(let s of sd){for(let y=s.y;y<s.y+s.h;y+=ws){if(iff){iff=false;continue}if(s===sd[sd.length-1]&&y+ws>=s.y+s.h){il=true;continue}let sc=color(s.pc);if(s.sc&&s.wt==='mixed'){let sc2=color(s.sc),bf=noise(y*0.1)*0.5+0.5;sc=lerpColor(sc,sc2,bf)}let r=red(sc)*0.8,g=green(sc)*0.8,b=blue(sc)*0.8;fill(r,g,b);noStroke();let rad=wt*window.prngRange(1.2,1.8),cx=f*2+window.prngRange(-2,2),cy=f*2+y+wt/2+window.prngRange(-1,1),sa=HALF_PI+window.prngRange(-0.2,0.2),ea=-HALF_PI+window.prngRange(-0.2,0.2);drawTexturedSelvedgeArc(cx,cy,rad,sa,ea,r,g,b,'left')}}let ifwr=true,ilwr=false;for(let s of sd){for(let y=s.y;y<s.y+s.h;y+=ws){if(ifwr){ifwr=false;continue}if(s===sd[sd.length-1]&&y+ws>=s.y+s.h){ilwr=true;continue}let sc=color(s.pc);if(s.sc&&s.wt==='mixed'){let sc2=color(s.sc),bf=noise(y*0.1)*0.5+0.5;sc=lerpColor(sc,sc2,bf)}let r=red(sc)*0.8,g=green(sc)*0.8,b=blue(sc)*0.8;fill(r,g,b);noStroke();let rad=wt*window.prngRange(1.2,1.8),cx=f*2+w+window.prngRange(-2,2),cy=f*2+y+wt/2+window.prngRange(-1,1),sa=-HALF_PI+window.prngRange(-0.2,0.2),ea=HALF_PI+window.prngRange(-0.2,0.2);drawTexturedSelvedgeArc(cx,cy,rad,sa,ea,r,g,b,'right')}}}
