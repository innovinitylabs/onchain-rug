...
contract RugNFTFacet is ERC721, ICreatorToken {
        ...
        returns (string memory)
    {
        ...
        // Get current dirt and aging levels from RugAgingFacet
        (bool success1, bytes memory data1) = address(this).staticcall(
            abi.encodeWithSignature("getDirtLevel(uint256)", tokenId)
        );
        require(success1, "Failed to get dirt level");
        uint8 dirtLevel = abi.decode(data1, (uint8));

        (bool success2, bytes memory data2) = address(this).staticcall(
            abi.encodeWithSignature("getAgingLevel(uint256)", tokenId)
        );
        require(success2, "Failed to get aging level");
        uint8 agingLevel = abi.decode(data2, (uint8));

        // HTML generation temporarily disabled for tokenURI testing

        // TEMPORARY: Very simple JSON for testing
        string memory json = Base64.encode(
            bytes(
                string(abi.encodePacked(
                    '{"name":"OnchainRug #', tokenId.toString(),
                    '","description":"OnchainRugs by valipokkann","attributes":[',
