...
// Global variables for the doormat generation
let currentSeed = 0;
let warpThickness = Math.floor(Math.random() * 6) + 1; // Random 1-6 for each generation
...
window.warpThickness = warpThickness; // Expose current warpThickness globally
...
function generateDoormatCore(seed) {
    ...
    // Re-expose warpThickness globally with current value
    window.warpThickness = warpThickness;
    console.log("🌐 Re-exposed warpThickness globally:", window.warpThickness);
