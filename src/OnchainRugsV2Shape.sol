...
contract OnchainRugsV2Shape is ERC721, ERC721URIStorage, Ownable {
    ...
    function generateOptimizedHTML(
        ...
        string memory part1 = string(abi.encodePacked(
            ...
            'function setup(){noiseSeed(seed);window.prngSeed=seed%2147483647;if(window.prngSeed<=0)window.prngSeed+=2147483646;window.prngNext=function(){window.prngSeed=(window.prngSeed*16807)%2147483647;return(window.prngSeed-1)/2147483646};window.prng=function(min,max){if(max===undefined)return min+window.prngNext()*(1-min);return min+window.prngNext()*(max-min)};window.prngChoice=function(a){return a[Math.floor(window.prngNext()*a.length)]};createCanvas(h+(f*4),w+(f*4)).parent("canvas-container");pixelDensity(2.5);updateTextColors();generateTextData();noLoop()}let w=800,h=1200,f=30,wp=wt,ts=2,lt,dt;'
        ));

        string memory part2 = string(abi.encodePacked(
            'function updateTextColors(){if(!p||!p.colors)return;let d=p.colors[0],l=p.colors[0],dv=999,lv=-1;for(let c of p.colors){let b=(red(color(c))+green(color(c))+blue(color(c)))/3;if(b<dv){dv=b;d=c}if(b>lv){lv=b;l=c}}dt=lerpColor(color(d),color(0),0.4);lt=lerpColor(color(l),color(255),0.3)}',
            'function draw(){background(222,222,222);push();translate(width/2,height/2);rotate(PI/2);translate(-height/2,-width/2);push();translate(f*2,f*2);for(let s of sd)drawStripe(s);if(stex&&tl>0)drawTextureOverlay(tl);pop();drawFringe();if(sdirt&&dl>0)drawDirtOverlay(dl);pop()};',
            'function drawStripe(s){let ws=wp+1,we=wt+1;for(let x=0;x<w;x+=ws)for(let y=s.y;y<s.y+s.height;y+=we){let wc=color(s.primaryColor),it=false;if(td.length)for(let tp of td)if(x>=tp.x&&x<tp.x+tp.width&&y>=tp.y&&y<tp.y+tp.height){it=true;break}let r=red(wc)+prng(-15,15),g=green(wc)+prng(-15,15),b=blue(wc)+prng(-15,15);if(it){let bb=(r+g+b)/3,tc=bb<128?lt:dt;r=red(tc);g=green(tc);b=blue(tc)}fill(constrain(r,0,255),constrain(g,0,255),constrain(b,0,255));rect(x+sin(y*0.05)*0.5,y,wp,we)}for(let y=s.y;y<s.y+s.height;y+=we)for(let x=0;x<w;x+=ws){let wc=color(s.primaryColor),it=false;if(td.length)for(let tp of td)if(x>=tp.x&&x<tp.x+tp.width&&y>=tp.y&&y<tp.y+tp.height){it=true;break}if(s.weaveType==="mixed"&&s.secondaryColor&&prng(0,1)>0.5)wc=color(s.secondaryColor);else if(s.weaveType==="textured")wc=lerpColor(color(s.primaryColor),color(255),noise(x*0.05,y*0.05)*0.15);let r=red(wc)+prng(-20,20),g=green(wc)+prng(-20,20),b=blue(wc)+prng(-20,20);if(it){let bb=(r+g+b)/3,tc=bb<128?lt:dt;r=red(tc);g=green(tc);b=blue(tc)}fill(constrain(r,0,255),constrain(g,0,255),constrain(b,0,255));rect(x,y+cos(x*0.05)*0.5,ws,wt)}for(let y=s.y;y<s.y+s.height;y+=we*2)for(let x=0;x<w;x+=ws*2){fill(0,0,0,40);rect(x+1,y+1,ws-2,we-2)}for(let y=s.y+we;y<s.y+s.height;y+=we*2)for(let x=ws;x<w;x+=ws*2){fill(255,255,255,30);rect(x,y,ws-1,we-1)}}function drawTextureOverlay(tl){push();blendMode(MULTIPLY);let hi=tl>1?80:30,ri=tl>1?40:20,rt=tl>1?0.5:0.6;for(let x=0;x<w;x+=2)for(let y=0;y<h;y+=2){fill(0,0,0,map(noise(x*0.02,y*0.02),0,1,0,hi));rect(x,y,2,2)}for(let x=0;x<w;x+=6)for(let y=0;y<h;y+=6){let r=noise(x*0.03,y*0.03);if(r>rt)fill(255,255,255,ri);else if(r<1-rt)fill(0,0,0,ri*0.8);rect(x,y,6,6)}if(tl>1)for(let x=0;x<w;x+=8)for(let y=0;y<h;y+=8)if(noise(x*0.01,y*0.01)>0.7){fill(0,0,0,15);rect(x,y,8,2)}pop()}function drawDirtOverlay(dl){push();translate(f*2,f*2);let di=dl>1?1:0.5,doo=dl>1?60:30;for(let x=0;x<w;x+=3)for(let y=0;y<h;y+=3)if(prng(0,1)>0.85*di){fill(prng(60,90),prng(40,60),prng(20,40),prng(doo*0.5,doo));ellipse(x,y,prng(1,4),prng(1,4))}for(let i=0;i<15*di;i++)fill(prng(40,70),prng(25,45),prng(15,30),prng(doo*0.3,doo*0.7)),ellipse(prng(0,w),prng(0,h),prng(8,20),prng(8,20));for(let x=0;x<w;x+=2)for(let y=0;y<h;y+=2)if(Math.min(x,y,w-x,h-y)<10&&prng(0,1)>0.7*di)fill(80,50,20,prng(10,25)),rect(x,y,2,2);pop()}function drawFringe(){drawFringeSection(f*2,f,w,f,"top");drawFringeSection(f*2,f*2+h,w,f,"bottom");drawSelvedge()}function drawFringeSection(x,y,w,h,s){let fs=w/12,sw=w/fs;for(let i=0;i<fs;i++){let sx=x+i*sw;if(!p||!p.colors)continue;let sc=color(prngChoice(p.colors)),r=red(sc)*0.7,g=green(sc)*0.7,b=blue(sc)*0.7;stroke(r,g,b);strokeWeight(prng(0.5,1.2));noFill();beginShape();for(let t=0;t<=1;t+=0.1){let yp=lerp(s==="top"?y+h:y,y,t*prng(0.8,1.2)),xo=sin(t*PI*prng(0.2,0.8))*prng(1,4)*t*prngChoice([-1,1])*prng(0.5,2);if(prng(0,1)<0.3)xo+=prng(-2,2);vertex(sx+prng(-sw/6,sw/6)+xo,yp)}endShape()}}function drawSelvedge(){let ws=wt+1;for(let s of sd)for(let y=s.y;y<s.y+s.height;y+=ws){if(y===s.y)continue;let sc=color(s.primaryColor);if(s.secondaryColor&&s.weaveType==="mixed")sc=lerpColor(sc,color(s.secondaryColor),noise(y*0.1)*0.5+0.5);let r=red(sc)*0.8,g=green(sc)*0.8,b=blue(sc)*0.8;fill(r,g,b);let rad=wt*prng(1.2,1.8),cx=f*2+prng(-2,2),cy=f*2+y+wt/2+prng(-1,1);arc(cx,cy,rad*2,rad*2,HALF_PI+prng(-0.2,0.2),-HALF_PI+prng(-0.2,0.2));cx=f*2+w+prng(-2,2);arc(cx,cy,rad*2,rad*2,-HALF_PI+prng(-0.2,0.2),HALF_PI+prng(-0.2,0.2))}}function generateTextData(){td=[];if(!tr||tr.length===0)return;const netr=tr.filter(r=>r&&r.trim());if(netr.length===0)return;const ws=wp+1,we=wt+1,sw=ws*ts,se=we*ts,cw=7*sw,ch=5*se,s=se,rs=cw*1.5,trw=netr.length*cw+(netr.length-1)*rs,bsx=(w-trw)/2;for(let ri=0;ri<netr.length;ri++){const rt=netr[ri],tw=cw,th=rt.length*(ch+s)-s,sx=bsx+ri*(cw+rs),sy=(h-th)/2;for(let i=0;i<rt.length;i++){const c=rt.charAt(i),cy=sy+(rt.length-1-i)*(ch+s),cd=cm[c.toUpperCase()]||cm[" "];for(let r=0;r<cd.length;r++)for(let col=0;col<cd[r].length;col++)if(cd[r][col]==="1")td.push({x:sx+r*sw,y:cy+(cd[r].length-1-col)*se,width:sw,height:se})}}}'
        ));
