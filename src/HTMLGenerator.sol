...
import "./OnchainRugs.sol";
import "lib/openzeppelin-contracts/contracts/utils/Strings.sol";

/**
 * @title HTMLGenerator
 * @dev Library for generating HTML content for NFTs
 */
library HTMLGenerator {
    using Strings for uint256;
    function generateHTML(
        OnchainRugs.RugData memory rug,
        uint8 dirtLevel,
        uint8 textureLevel
    ) internal pure returns (string memory) {
        string memory jsConfig = string(abi.encodePacked(
            'let w=800,h=1200,f=30,wt=8,wp=', uint256(rug.warpThickness).toString(),
            ',ts=2,lt,dt,p=', rug.minifiedPalette,
            ',sd=', rug.minifiedStripeData,
            ',tr=', encodeTextRows(rug.textRows),
            ',td=[],dl=', uint256(dirtLevel).toString(),
            ',tl=', uint256(textureLevel).toString(),
            ',s=', rug.seed.toString(),
            ',cm=', rug.filteredCharacterMap, ';'
        ));

        return string(abi.encodePacked(
            '<!DOCTYPE html><html><head><meta charset=UTF-8><meta name=viewport content="width=device-width,initial-scale=1"><title>OnchainRug #',
            rug.seed.toString(),
            '</title><script src=https://cdnjs.cloudflare.com/ajax/libs/p5.js/1.7.0/p5.min.js></script><style>body{display:flex;justify-content:center;align-items:center}#defaultCanvas0{width:100%!important;height:auto!important;}</style></head><body><div id=canvas-container></div><script>',
            jsConfig,
            getJavaScriptAlgorithm(),
            '</script></body></html>'
        ));
    }
    ...
    function encodeTextRows(string[] memory textRows) internal pure returns (string memory) {
        ...
        for (uint256 i = 0; i < textRows.length; i++) {
            if (i > 0) r = string(abi.encodePacked(r, ","));
            r = string(abi.encodePacked(r, '"', textRows[i], '"'));
        }
