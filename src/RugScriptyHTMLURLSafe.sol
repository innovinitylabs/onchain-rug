...
import {IRugScriptyHTMLURLSafe} from "./IRugScriptyHTMLURLSafe.sol";
import {RugScriptyCore} from "./RugScriptyCore.sol";
import {RugDynamicBuffer} from "./RugDynamicBuffer.sol";

contract RugScriptyHTMLURLSafe is RugScriptyCore, IRugScriptyHTMLURLSafe {
    ...
    function getHTMLURLSafe(
        ...
        bytes memory htmlFile = RugDynamicBuffer.allocate(
            _rugGetHTMLURLSafeBufferSize(requestBufferSize)
        );

        // data:text/html,
        htmlFile.appendSafe(RUG_DATA_HTML_URL_SAFE);

        // <html>
        htmlFile.appendSafe(RUG_HTML_OPEN_URL_SAFE);

        // <head>
        htmlFile.appendSafe(RUG_HEAD_OPEN_URL_SAFE);
        if (htmlRequest.headTags.length > 0) {
            _rugAppendHTMLURLSafeTags(htmlFile, htmlRequest.headTags);
        }
        htmlFile.appendSafe(RUG_HEAD_CLOSE_URL_SAFE);
        // </head>

        // <body>
        htmlFile.appendSafe(RUG_BODY_OPEN_URL_SAFE);
        if (htmlRequest.bodyTags.length > 0) {
            _rugAppendHTMLURLSafeTags(htmlFile, htmlRequest.bodyTags);
        }
        htmlFile.appendSafe(RUG_HTML_BODY_CLOSED_URL_SAFE);
        // </body>
        // </html>
     ...
     *
     *      1. `rugTagOpenCloseForHTMLTagURLSafe()` will convert the wrap to the following
     *      - <script>  =>  %253Cscript%2520src%253D%2522data%253Atext%252Fjavascript%253Bbase64%252C
     *      - </script> =>  %2522%253E%253C%252Fscript%253E
     *
     *      2. `_rugAppendHTMLTag()` will base64 encode the script to the following
     ...
     */
    function _rugAppendHTMLURLSafeTags(
        ...
        unchecked {
            do {
                ...
                (htmlTag.tagType == RugHTMLTagType.script)
                    ? _rugAppendHTMLTag(htmlFile, htmlTag, true)
                    : _rugAppendHTMLTag(htmlFile, htmlTag, false);
