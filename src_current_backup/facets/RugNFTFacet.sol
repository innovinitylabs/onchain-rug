 ...
 */
contract RugNFTFacet is ERC721 {
    ...
    constructor() ERC721("OnchainRugs", "RUGS") {
        LibTransferSecurity.transferSecurityStorage().transferValidator = DEFAULT_TRANSFER_VALIDATOR;
        LibTransferSecurity.transferSecurityStorage().securityPolicyId = 1; // Default policy
        LibTransferSecurity.transferSecurityStorage().enforcedTransfers = true;
        LibTransferSecurity.transferSecurityStorage().initialized = true;
    }
    ...
    function mintRug(
        ...
        // Mint NFT (use _mint for open mints without receiver checks)
    ...
    function getTransferValidator() external view returns (address) {
        return LibTransferSecurity.transferSecurityStorage().transferValidator;
    }

    function getSecurityPolicy() external view returns (CollectionSecurityPolicy memory) {
        address validator = LibTransferSecurity.transferSecurityStorage().transferValidator;
        uint120 policyId = uint120(LibTransferSecurity.transferSecurityStorage().securityPolicyId);
        return ICreatorTokenTransferValidator(validator).getCollectionSecurityPolicy(policyId);
    }

    function getWhitelistedOperators() external view returns (address[] memory) {
        CollectionSecurityPolicy memory policy = this.getSecurityPolicy();
        address validator = LibTransferSecurity.transferSecurityStorage().transferValidator;
        return ICreatorTokenTransferValidator(validator).getWhitelistedOperators(policy.operatorWhitelistId);
    }

    function getPermittedContractReceivers() external view returns (address[] memory) {
        CollectionSecurityPolicy memory policy = this.getSecurityPolicy();
        address validator = LibTransferSecurity.transferSecurityStorage().transferValidator;
        return ICreatorTokenTransferValidator(validator).getPermittedContractReceivers(policy.permittedContractReceiverAllowlistId);
    }

    function isOperatorWhitelisted(address operator) external view returns (bool) {
        CollectionSecurityPolicy memory policy = this.getSecurityPolicy();
        address validator = LibTransferSecurity.transferSecurityStorage().transferValidator;
        return ICreatorTokenTransferValidator(validator).isOperatorWhitelisted(policy.operatorWhitelistId, operator);
    }

    function isContractReceiverPermitted(address receiver) external view returns (bool) {
        CollectionSecurityPolicy memory policy = this.getSecurityPolicy();
        address validator = LibTransferSecurity.transferSecurityStorage().transferValidator;
        return ICreatorTokenTransferValidator(validator).isContractReceiverPermitted(policy.permittedContractReceiverAllowlistId, receiver);
    }

    function isTransferAllowed(address from, address to, uint256 tokenId) external view returns (bool) {
        address validator = LibTransferSecurity.transferSecurityStorage().transferValidator;
        uint120 policyId = uint120(LibTransferSecurity.transferSecurityStorage().securityPolicyId);
        return ICreatorTokenTransferValidator(validator).isTransferAllowed(address(this), policyId, from, to, tokenId);
    }

    function _beforeTokenTransfer(address from, address to, uint256 tokenId, uint256 batchSize) internal override {
        super._beforeTokenTransfer(from, to, tokenId, batchSize);

        // Skip validation for mints
        if (from == address(0)) return;

        // ERC721-C transfer validation
        if (LibTransferSecurity.transferSecurityStorage().enforcedTransfers) {
            address validator = LibTransferSecurity.transferSecurityStorage().transferValidator;
            uint120 policyId = uint120(LibTransferSecurity.transferSecurityStorage().securityPolicyId);
            ICreatorTokenTransferValidator(validator).applyCollectionTransferPolicy(address(this), policyId, from, to, tokenId);
        }
    }
}
